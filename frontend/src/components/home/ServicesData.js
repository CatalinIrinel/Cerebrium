const data = {
  services: [
    {
      img: '/images/Bg2.jpg',
      alt: 'brain mapping cerebrium afectiuni',
      title: 'Afecțiuni',
      descriere:
        'Multiple afecțiuni pot fi ținute sub control cu ajutorul neurofeedback-ului cu frecvențe foarte joase. Puteți vedea aici lista completă și beneficiile aduse în urma sedințelor de neurofeedback.',
      buttonLink: '/afectiuni',
      buttonText: 'Mai multe detalii',
    },
    {
      img: '/images/NeuroBG.jpg',
      alt: 'neurofeedback ilf cerebrium',
      title: 'Neurofeedback ILF',
      descriere:
        'Neurofeedback-ul este o metodă de antrenare directă a funcțiilor cerebrale cu ajutorul căreia creierul învață să funcționeze mai eficient. Autoreglarea permite sistemului nervos central să funcționeze mai bine. Numeroase afecțiuni au la bază lipsa unei reglări naturale a creierului și a proceselor implicate.',
      buttonLink: '/ce-este-neurofeedback-ul',
      buttonText: 'Mai multe detalii',
    },
    {
      img: '/images/frecventa.jpeg',
      alt: 'frecventa neurofeedback cerebrium',
      title: 'Frecventa Optima',
      descriere:
        'Ce înseamnă frecvența optimă și care este aplicabilitatea acesteia? Vezi despre beneficiile și importanța aflării frecvenței optime pentru creierul tău.',
      buttonLink: '/frecventa-optima',
      buttonText: 'Mai multe detalii',
    },
  ],
};

export default data;
